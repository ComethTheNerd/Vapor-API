...
public class Hide<self extends Hide<self>> extends Anim<self> {

	protected Fade fader;
	protected static final long __DURATION = 0L;
	public static final String KEEP_SPACE = "keepSpaced";
	protected static final boolean __KEEP_SPACE = false;

	private int lastHash = 0;

	public Hide(VaporView<? extends View, ?> view, VaporBundle options) {
		super(view, options);
	}

	public Hide(VaporView<? extends View, ?> view) {
		super(view);
	}

	public boolean keepSpace() {
		return prop(KEEP_SPACE, __KEEP_SPACE);
	}

	public self keepSpace(boolean keepSpace) {
		options.put(KEEP_SPACE, keepSpace);
		return (self) this;
	}
	...
	public void run() {
		...
		options.put(Fade.FROM, 1F);
		options.put(Fade.TO, 0F);
	...
	protected final vapor.listeners.animation.animator.$anim combineListeners(
		...
		return new $anim() {
			...
			public void onAnimationEnd(Animator animation) {
				...
				// User can decide if the view takes up layout space after
				// hiding
				view.viz(prop(KEEP_SPACE, __KEEP_SPACE) ? View.INVISIBLE
						: View.GONE);
